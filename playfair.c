...
char dec(char []);
void playfair();
char key[100],play[5][5],ct[255],msg[255],plaintxt[255],ctext[255];
void main(){
	...
	if(msg[i]!=' ')
		msg[count++]=msg[i];
	msg[count]='\0';
	for(i=0;i<size;i=+2){ if(msg[i]==msg[i+1]){for(j=size;j>i;j--)
		{msg[j]=msg[size-1];size--;}msg[j+1]='x';
	}
	}
	size=strlen(msg);
...
void choicefill(){
	...
	do{
		...
		scanf("%d",&choice);
		switch(choice){
			case 1:enc(msg); flag=1; break;
			case 2:if(flag==1) dec(ct);
			else printf("first perform enc process");
			break;
			case 0: exit(1); break;
			default: printf("\nplease enter valid");
		}
...
char dec(char pt[]){
	...
	for(i=0;i<strlen(pt);){
		...
		putchar(ct[i]);putchar(ct[j]);}
		else{ct[i]=play[r1][c2];ct[j]=play[r2][c1]; putchar(ct[i]);
		putchar(ct[j]);}
		i=i+2;j=j+2;
	} 
	printf("\nPlain text msg:%s",ct);
...
void playfair(){
	...
	for(i=0;i<size;i++){
		...
		for(j=i+1;j<size;)
			{if(key[j]==key[i]){
				for(k=j;k<size;k++){key[k]=key[k+1];} size--;}
			else j++;}}
	...
	for(i=0,k=0;i<5;i++){
		...
		{
			for(j=0;j<5;j++){
				printf(" "); putchar(play[i][j]);printf("\n");
			}
		}
}
